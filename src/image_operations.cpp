...
cv::Mat ImageOperations::grayscale(const cv::Mat &image,
                                   const int conversion_type) {
  cv::Mat res;
  cv::cvtColor(image, res, conversion_type);
  return res;
}

cv::Mat ImageOperations::sobel(const cv::Mat &image, const int conv_type,
...
cv::Mat ImageOperations::blendImages(const cv::Mat &image1,
                                     const cv::Mat &image2,
                                     const double alpha) {
  cv::Mat res;
