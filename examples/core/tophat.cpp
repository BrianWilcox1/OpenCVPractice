...
int main(int argc, char **argv) {
  if (argc != 2) {
    printf("usage: tophat.out <Image_Path>\n");
    return -1;
  }
  cv::Mat image;
  ...
  auto opened_output =
      morph_ops.dilation(eroded_output, dilation_type, dilation_size);
  cv::Mat res;
  cv::subtract(image, opened_output, res);

  cv::imwrite("tophat_image.jpg", res);
